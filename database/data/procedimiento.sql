...
(
    ...
    ifnull((SELECT SUM(ddv.cantidad) FROM cotizacion_documento_detalles ddv INNER JOIN cotizacion_documento dv ON ddv.documento_id = dv.id INNER JOIN lote_productos lp ON ddv.lote_id = lp.id WHERE dv.fecha_documento < fecini AND dv.estado = 'ACTIVO' AND lp.producto_id = p.id AND ddv.eliminado = '0'),0) +
    ifnull((SELECT SUM(ddv.cantidad) FROM cotizacion_documento_detalles ddv INNER JOIN cotizacion_documento dv ON ddv.documento_id = dv.id INNER JOIN lote_productos lp ON ddv.lote_id = lp.id WHERE dv.fecha_documento < fecini AND dv.estado = 'ACTIVO' AND lp.producto_id = p.id AND dv.tipo_venta != '129' AND dv.convertir != '' AND ddv.eliminado = '0'),0) -
...
(
    ifnull((SELECT SUM(vdd.cantidad) from cotizacion_documento_detalles vdd INNER JOIN cotizacion_documento vd ON vdd.documento_id = vd.id INNER JOIN lote_productos lp ON vdd.lote_id = lp.id WHERE vd.fecha_documento >= fecini and vd.fecha_documento <= fecfin AND vd.estado = 'ACTIVO' AND lp.producto_id = p.id AND vdd.eliminado = '0'),0) -
    ifnull((SELECT SUM(vdd.cantidad) from cotizacion_documento_detalles vdd INNER JOIN cotizacion_documento vd ON vdd.documento_id = vd.id INNER JOIN lote_productos lp ON vdd.lote_id = lp.id WHERE vd.fecha_documento >= fecini and vd.fecha_documento <= fecfin AND vd.estado = 'ACTIVO' AND lp.producto_id = p.id AND vd.tipo_venta != '129' AND vd.convertir != '' AND vdd.eliminado = '0'),0)
) AS VENTAS,

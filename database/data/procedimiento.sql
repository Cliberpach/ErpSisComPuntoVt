...
(
    ifnull((SELECT sum(ddc.cantidad) from compra_documento_detalles ddc INNER JOIN compra_documentos dc ON ddc.documento_id = dc.id WHERE dc.fecha_emision < fecini AND ddc.producto_id = p.id AND dc.estado != 'ANULADO'),0) + ifnull((SELECT sum(dni.cantidad) from detalle_nota_ingreso dni INNER JOIN nota_ingreso ni ON dni.nota_ingreso_id = ni.id WHERE ni.fecha < fecini AND dni.producto_id = p.id AND ni.estado != 'ANULADO'),0) -
    ...
    ifnull((SELECT SUM(dns.cantidad) FROM detalle_nota_salidad dns INNER JOIN nota_salidad ns ON dns.nota_salidad_id = ns.id WHERE ns.fecha < fecini AND ns.estado != 'ANULADO' AND dns.producto_id = p.id),0) + ifnull((SELECT SUM(ned.cantidad) FROM nota_electronica_detalle ned INNER JOIN nota_electronica ne ON ned.nota_id = ne.id INNER JOIN cotizacion_documento_detalles cdd ON cdd.id = ned.detalle_id INNER JOIN lote_productos lpn ON lpn.id = cdd.lote_id WHERE ne.fechaEmision < fecini AND ne.estado != 'ANULADO' AND lpn.producto_id = p.id),0)
) as STOCKINI,
